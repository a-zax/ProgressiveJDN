...
const MissingBankSuggestion = () => {
	...
	// Reverse geocode method to get address based on latitude and longitude
	const reverseGeocode = async (coordinates) => {
		try {
			...
			if (result.data.results.length > 0) {
				...
				setAddress("Current Location Fetching");
			}
		} catch (err) {
	...
	const handleRegionChange = async (region) => {
		...
		await reverseGeocode(region); // Get address after updating the location
	};
						...
						<View style={styles.centeredView}>
							<View style={styles.modalView}>
								...
								{res !== null ? (
									<Text style={styles.trackID}>
										Track ID:{" "}
										{res.data.uid.substring(0, 8).toUpperCase()}
									</Text>
								) : null}
								<View style={styles.modalButtons}>
									<Pressable
