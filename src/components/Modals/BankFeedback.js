...
import {
  ...
  const BankFeedback = () => {
	...
	const [unique_id, setUnique_id] = useState("");
	const [error, setError] = useState("");
	const [uri, setUri] = useState(null); // useState for uri
	const [recording, setRecording] = useState(false); // useState for recording
  
	const handleSubmit = async () => {
	  try {
		setIsLoading(true);
  
		// Hardcoded response data instead of API call
		const data = {
		  success: true, // Simulating a successful submission
		...
		flashMessage(err?.response?.data || "An unexpected error occurred.", "danger");
	  } finally {
		setIsLoading(false);
	  }
	};
	  ...
	  <View style={styles.container}>
		<View>
		  ...
		  <ScrollView
			...
			showsVerticalScrollIndicator={false}
		  >
			<Text style={{ color: "#e35944", fontSize: 16.5, fontWeight: "600", marginLeft: 2, marginBottom: 2 }}>
			  {error}
			</Text>
			<InputField
			  inputname="Location"
